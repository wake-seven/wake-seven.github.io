...
function showHintArrow(ti,dir,both=false,persistent=false){
  ...
  // Start on the outer side of each pivot. At this point the intended pivot is
  // always the nearest one, so grabbing the arrow cannot select its neighbour.
  const boardCenter=CELL[3];
  const outwardAngle=Math.atan2(t.y-boardCenter.y,t.x-boardCenter.x)*180/Math.PI;
  const startAngle=outwardAngle,endAngle=startAngle+dir*120;
  const shaftEnd=endAngle-dir*22;
  const point=a=>{
  ...
  const nx=-ay, ny=ax;
  const w1={x:base.x+nx*9,y:base.y+ny*9};
  const w2={x:base.x-nx*9,y:base.y-ny*9};
  let secondHead='';
  if(both){
    const sb=point(startAngle+dir*22), sq=startAngle*Math.PI/180;
    const sax=Math.sin(sq)*dir, say=-Math.cos(sq)*dir;
    const snx=-say, sny=sax;
    const sw1={x:sb.x+snx*9,y:sb.y+sny*9};
    const sw2={x:sb.x-snx*9,y:sb.y-sny*9};
  ...
  const arcD='M '+start.x.toFixed(2)+' '+start.y.toFixed(2)
    +' A '+r+' '+r+' 0 0 '+(dir>0?1:0)+' '+base.x.toFixed(2)+' '+base.y.toFixed(2);
  const g=document.createElementNS(NS_,'g');
  g.setAttribute('class','hint-arrow'+(persistent?' tutorial':''));
  g.innerHTML=(persistent?'<path class="arc-border" d="'+arcD+'"/>':'')
    +'<path class="arc" pathLength="100" d="'+arcD+'"/>'
    +(persistent?'<path class="arc-glow" pathLength="100" d="'+arcD+'"/>':'')
  ...
  svg.appendChild(g);
  if(!persistent)setTimeout(()=>{if(g.isConnected){
    clearHintVisuals();
    if(usesHiddenRemaining()){
      fourthHintPreview=false;fourthDistanceRevealed=true;fourthHintDistance=null;
...
function showBestMoveHint(persistent=false){
  ...
  if(usesHiddenRemaining()){
    fourthHintPreview=true;fourthDistanceRevealed=false;fourthHintDistance=Math.max(0,d-1);
    showRemaining(remainingForDisplay(d));
  }
  for(let ti=0;ti<TRI.length;ti++){
    for(const dir of [1,-1]){
      ...
      showHintArrow(ti,dir,false,persistent);
      return;
    }
  }
}
function cancelTutorialHint(preserveSelection=false){
  tutorialHintTimers.forEach(clearTimeout);
  tutorialHintTimers=[];
  clearAxisGuide();
  clearHintVisuals(preserveSelection);
  svg.querySelectorAll('.pivot.hi').forEach(el=>el.classList.remove('hi'));
}
function scheduleStageOneTutorial(){
  cancelTutorialHint();
}
function scheduleBasicLessonAssist(){
  cancelTutorialHint();
}
$('hint').addEventListener('click',()=>{
  cancelTutorialHint();
  if(isFourthVolume())revealFourthDistance();
  else showBestMoveHint();
});
...
function rankFrameSvg(text,locked=false,index=0,doubleFrame=false,dimmed=false){
  ...
  // 「覚者」は大きく、英語の Awakened は枠内に収まるサイズにする。
...
function renderRankList(){
  ...
  const earnedCount=second
    ?(awakenedGranted?sequence.length:Math.max(0,highest+1))
  ...
  list.innerHTML='';
  // 未到達の節目名まで先に見えてしまわないよう、既に得た称号＋次の一つだけを見せる（悟りも同様に段階解放）。
...
function openRankDialog(returnTarget=null){
  ...
  // 一覧を開いた時は、最新到達周ではなく現在遊んでいる周を見せる。
...
function configureMessageReviewType(entry){
  const content=messageReviewContent(entry);
  const messageClearEntry=content||(entry.extra?clearContentAt(true,entry.index):null);
  ...
  return messageClearEntry;
}
function configureMessageReviewLinks(entry,messageClearEntry){
  const twoMoveCard=messageClearEntry?.twoMoveCard;
  const guideCard=messageClearEntry?.guideCard;
  const rankLink=messageClearEntry?.link==='rank';
  const tipsLink=messageClearEntry?.link==='tips';
  const patternsLink=messageClearEntry?.link==='patterns';
  const link=$('messageRankLink');
  link.hidden=twoMoveCard===undefined&&!guideCard&&!rankLink&&!tipsLink&&!patternsLink;
  link.dataset.target=tipsLink?'tips':patternsLink?'patterns':twoMoveCard!==undefined?'card':guideCard?'guide':'rank';
...
function resetMessageReviewView(entry,view){
  const roadmap=$('messageRoadmap'),roadmapNote=$('messageRoadmapNote');
  ...
  renderMasteryBoard('messageMasteryBoard',MESSAGE_MASTERY_BOARD_MASTERS.includes(entry.master),MESSAGE_TILTED_BOARD_MASTERS.includes(entry.master)?'satori-tilted':'gold');
  roadmap.hidden=!entry.master||!MESSAGE_ROADMAP_MASTERS.includes(entry.master);
  roadmapNote.hidden=true;view.boardNote.hidden=true;view.rules.hidden=true;
  view.seal.hidden=!entry.master;
  view.seal.classList.add('rank-seal');
  view.seal.classList.remove('rank-frame-seal','second-lap-mark');
...
function renderMessageReview(){
  ...
  const illustration=$('messageIllustration'),lessonCopy=$('messageTwoMoveLessonCopy'),lessonRule=$('messageTwoMoveLessonRule'),roadmap=$('messageRoadmap'),seal=$('messageMasterSeal');
...
function renderMasterRoadmap(kind){
  const roadmap=$('masterRoadmap');
  const roadmapNote=$('masterRoadmapNote');

...
const APP_VERSION='2026.09.02-15:44';
...
function renderApplicationTargetCells(){
  ...
  if(application&&!isSolved()&&!targets.size){
    ...
    if(move){
      // 一手目の後に残る最短一手を調べ、その操作対象を最終目標として示す。
      // これにより、現在つかむ三枚ではなく、次に同じ向きで寝る三枚を強調できる。
      const after=rollOnce(ori,move.ti,move.dir),nextMove=lessonBestMove(enc(after));
      (nextMove?TRI[nextMove.ti]:null)?.cells.forEach(cell=>targets.add(cell));
    }

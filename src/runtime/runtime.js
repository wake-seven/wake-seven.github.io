...
const APP_VERSION='2026.09.02-15:51';
...
function renderApplicationTargetCells(){
  ...
  if(application&&!isSolved()&&!targets.size){
    ...
    if(move){
      // 一手目の後の盤面から、同じ向きで寝ている三枚を全セル対象で探す。
      // 最終操作の三角形ではなく、問題中に見せたい目標グループそのものを示す。
      const after=rollOnce(ori,move.ti,move.dir);
      const groups=[1,2].map(orientation=>after.reduce((cells,value,cell)=>{
        if(value===orientation)cells.push(cell);
        return cells;
      },[]));
      const exact=groups.find(cells=>cells.length===3);
      const target=exact||groups.filter(cells=>cells.length>3).sort((a,b)=>b.length-a.length)[0];

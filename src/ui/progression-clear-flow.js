...
// クリア後フローの段階。表示中のDOMではなく、この状態を見れば
// 「いま何を待っているか」「次に何が起きるか」を追跡できる。
const CLEAR_FLOW_PHASE=Object.freeze({
  idle:'idle',
  animation:'clear-animation',
  animationPending:'clear-animation-pending',
  dialog:'clear-dialog',
...
function scheduleClearFlowDialog(callback,delay,cycle=clearFlowCycle){
  if(clearFlowPhase!==CLEAR_FLOW_PHASE.animation)return false;
  ...
  setUiEffectTimer('clear-transition','show-dialog',()=>{
    if(cycle!==clearFlowCycle||clearFlowPhase!==CLEAR_FLOW_PHASE.animationPending)return;
    callback();
  },delay);
  return true;
}
...
function dispatchClearFlowAction(action){
  if(action!==CLEAR_FLOW_ACTION.next||![CLEAR_FLOW_PHASE.dialog,CLEAR_FLOW_PHASE.content].includes(clearFlowState.phase))return false;
  ...
  hideGameDialogs();
  if(['master-dialog','before-dialog'].includes(route.kind)){
    setClearFlowPhase(CLEAR_FLOW_PHASE.nextDialog,'open-next-dialog',{route});
    if(route.kind==='master-dialog')return showMasterDialog(route.section);
    return requestProgressionDialog('chain',{name:clearContentBefore(false,nextStageIndex).dialog},'clear');
  }
  setClearFlowPhase(CLEAR_FLOW_PHASE.playing,'open-next-playing',{route});

...
function finishDrag(e,cancel=false,forcedTurns=null){
  ...
  if(isMode('tutorial')&&!dir&&dg.maxAbsDeg>=120){
    ...
    if(expected&&dg.ti!==expected.ti){
      ...
      animateGuidedBasicRewind(dg,{start:dg.deg,end:0,tutorial:true});
      return;
    }
  }
...
function animateGuidedBasicRewind(dg,rewindOverride=null){
  if(matchMedia('(prefers-reduced-motion: reduce)').matches){
    paint();haptic(8);return;
  }
  const rewindPath=rewindOverride?.start!==undefined?rewindOverride:guidedRewindPath(dg.deg);
  const rewindStart=rewindPath.start,rewindEnd=rewindPath.end;
  const tutorialRewind=rewindOverride?.tutorial===true;
  ...
  const frame=now=>{
    ...
    finishBoardAnimationSession(session);
    if(tutorialRewind){
      setUiEffectTimer('tutorial-rewind','cue',showTutorialCue,150);
    }else{

...
function animateGuidedBasicRewind(dg){
  ...
  // 360°付近だけは反対側の小さい角度へ折り返す。
  // 240°などの大きな往復は、最短の-120°へ置き換えず、回した方向へ戻す。
  let rewindDeg=dg.deg%360;
  const nearZeroThreshold=30;
  if(Math.abs(rewindDeg)>360-nearZeroThreshold)rewindDeg-=Math.sign(rewindDeg)*360;
  // すでに初期角度に戻っている場合は、空回りするアニメーションを作らない。
  // pointerの丸め誤差や数度の揺れは0°相当として、不要な小回転を見せない。
  if(Math.abs(rewindDeg)<nearZeroThreshold){paint();haptic(8);return;}
  setBoardBusy(true);
  ...
  const frame=now=>{
    if(start===null)start=now;
    const progress=Math.min(1,(now-start)/duration);
    const deg=rewindDeg*(1-ease(progress));

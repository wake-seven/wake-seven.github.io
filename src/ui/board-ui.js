...
function animateGuidedBasicRewind(dg){
  ...
  // 360°未満の往復は実際に回した角度をそのまま戻し、急な小回転へ短縮しない。
  // 360°を超えた場合だけ余剰角へ戻す（390°なら30°）。
  const rawDeg=dg.deg;
  let rewindDeg=rawDeg%360;
  if(Math.abs(rawDeg)<360&&Math.abs(rewindDeg)<30)rewindDeg=rawDeg;
  // 360°ちょうどなど、見かけ上すでに初期角度に戻っている場合だけ省略する。
  if(Math.abs(rewindDeg)<0.5){paint();haptic(8);return;}
  setBoardBusy(true);

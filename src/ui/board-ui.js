...
function finishDrag(e,cancel=false,forcedTurns=null){
  ...
  if(isMode('tutorial')&&dir&&!tutorialMoveAllowed(dg.ti,dir)){
    ...
    animateTutorialRewind(createTutorialRewindModel({startAngle:dg.deg,endAngle:target,direction:dir,pivot:dg.kc,items:dg.items.map(({from,turn,dx,dy})=>({from,turn,dx,dy})),duration:720,cue:TUTORIAL_STEPS[tutorialStep]?.cue||''}),dg.items);
    return;
  }
  ...
  if(isMode('tutorial')&&!dir&&dg.maxAbsDeg>=120){
    ...
    if(expected&&dg.ti!==expected.ti){
      ...
      const rewindDir=dg.rawDeg>=0?1:-1;
      animateTutorialRewind(createTutorialRewindModel({startAngle:dg.deg,endAngle:0,direction:rewindDir,pivot:dg.kc,items:dg.items.map(({from,turn,dx,dy})=>({from,turn,dx,dy})),duration:720,cue:TUTORIAL_STEPS[tutorialStep]?.cue||''}),dg.items);
      return;
    }
  }
...
function animateTutorialRewind(model,visualItems){
  ...
  for(const item of visualItems){
    item.el.style.transform=orbitTransform(item,0,model.pivot);
    group.appendChild(item.el);
  }
  setTutorialRewindTimer(session,()=>{
    if(!isMode('tutorial'))return;
    renderTutorialRewindAppearance(visualItems,model.direction);haptic(6);
  },245);
  setTutorialRewindTimer(session,()=>{
    if(!isMode('tutorial'))return;
    renderTutorialRewindAppearance(visualItems,0);
  },440);
  group.style.transformOrigin=model.pivot.x+'px '+model.pivot.y+'px';

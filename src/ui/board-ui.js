...
function animateGuidedBasicRewind(dg){
  ...
  // 360°未満の往復は実際に回した角度をそのまま戻す。
  // 360°に近い場合は0°へ戻さず、360°まで進めてから正規化する。
  // 360°を超えた場合だけ余剰角へ戻す（390°なら30°→0°）。
  const rawDeg=dg.deg;
  let rewindStart=rawDeg%360,rewindEnd=0;
  if(Math.abs(rawDeg)<360&&Math.abs(rewindStart)>330)rewindEnd=Math.sign(rewindStart)*360;
  else if(Math.abs(rawDeg)<360&&Math.abs(rewindStart)<30)rewindStart=rawDeg;

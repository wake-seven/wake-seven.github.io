...
function renderQuizInto(ids,quiz){const root=$(ids.root),options=$(ids.options),note=$(ids.note);root.classList.remove('quiz-success');const {answers,correct}=quizPresentation(quiz);$(ids.title).textContent=tr('quizTitle');$(ids.question).textContent=quiz.q;note.textContent='';while(options.firstChild)options.removeChild(options.firstChild);answers.forEach((answer,index)=>{const button=document.createElement('button');button.className='quiz-option';button.type='button';button.textContent=answer;button.addEventListener('click',()=>{[...options.children].forEach(item=>item.disabled=true);options.children[correct].classList.add('correct');if(index===correct){note.textContent=tr('quizCorrect')+'　'+quiz.note;celebrateQuiz(root);}else{button.classList.add('wrong');note.textContent=tr('quizWrong')+'　'+quiz.note;}});options.appendChild(button);});}

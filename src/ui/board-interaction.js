...
function startBoardAnimationSession(type,pointerId=null,cleanup=()=>{}){
  ...
  const session={id:++boardAnimationSequence,type,pointerId,phase:BOARD_ANIMATION_PHASE.starting,startedAt:performance.now(),cancelled:false,frameHandle:0,cleanup,cleaned:false,resources:[],resourceKinds:[]};
...
function setBoardAnimationPhase(session,phase){if(!session||session.cleaned)return false;if(!Object.values(BOARD_ANIMATION_PHASE).includes(phase))return false;session.phase=phase;return true;}
function getBoardAnimationSession(){return boardAnimationSession;}
function requestBoardAnimationFrame(session,callback){if(!isBoardAnimationSessionActive(session))return 0;setBoardAnimationPhase(session,BOARD_ANIMATION_PHASE.running);session.frameHandle=requestAnimationFrame(now=>{session.frameHandle=0;if(isBoardAnimationSessionActive(session))callback(now);});return session.frameHandle;}
...
function finishBoardAnimationSession(session){
  ...
  if(!session.cancelled)setBoardAnimationPhase(session,BOARD_ANIMATION_PHASE.finishing);
  ...
  for(const entry of session.resources.splice(0)){
    ...
    entry.disposed=true;
    try{entry.dispose(entry.resource);}catch(_){/* 後処理失敗で次の後処理を止めない */}
  }
  session.cleanup?.();

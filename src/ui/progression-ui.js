...
function renderStagePicker(){
  ...
  for(let i=0;i<pageSize;i++){
    ...
    b.classList.toggle('current',pickerLap===navigation.lap&&!isMode('speed')&&(isPrimary?!isMode('mastery')&&!isMode('satori')&&navigation.stageIndex===index:isMode('mastery')&&navigation.masteryIndex===index));
...
function clearDialogShowsStageCount(){
  const {navigation}=readProgressionContext();
  return navigation.mode==='mastery'||(navigation.mode!=='satori'&&navigation.mode!=='speed'&&['intro','basic','application','development'].includes(primarySectionPosition(navigation.stageIndex).section.id));
}
...
function clearDialogHeading(){
  const {navigation}=readProgressionContext();
  return tr(navigation.mode==='mastery'||navigation.mode==='satori'?'optimalClear':'clear');
}
...
function showClearDialog(contextOverride=null){
  ...
  const {navigation}=readProgressionContext();
  const clearContext=contextOverride?.mode?contextOverride:createClearTransitionContext(navigation.stageIndex+1);

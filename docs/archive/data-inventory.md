...
この文書は、データと処理をさらに分離する前の調査メモです。現行コードの配置・互換仕様ではありません。旧並び・移行用配列に関する記述は当時の検討履歴であり、現在の保存データ移行を意味しません。公開物は引き続き `scripts/build-index.mjs` で生成する単一の `index.html` です。

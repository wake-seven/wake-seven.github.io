...
この文書は、旧実装との互換処理と、現在の画面から参照されていないデータを整理するためのメモです。ここに記録したものは、動作確認なしに削除しません。
...
新しい統合状態は `wake7-state-vnext` に保存します。`WakeSevenState.migrateLegacy()` は、旧キーを読み取り、`navigation` / `board` / `progress` / `unlocks` / `settings` / `speed` に変換してから新キーへ書き込みます。既存プレイヤーのデータを失わないため、旧キーは当面読み取り可能なままにします。

...
公開版の識別値は `src/runtime/runtime.js` の `APP_VERSION` を唯一の定義元とします。形式は `YYYY.MM.DD-HH:mm`（ローカル時刻）です。Aboutダイアログにはこの値へ `v` を付けて表示し、`npm run build` で生成される単体 `index.html` にも同じ値を埋め込みます。

ソースを公開・デプロイする変更では、ビルド前に `APP_VERSION` を分単位で更新します。ドキュメント側へ現在値を重複記載せず、実際の値はソースとAboutダイアログを確認します。

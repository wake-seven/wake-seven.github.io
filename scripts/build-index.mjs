...
const appModuleFiles = ['domain-board.js', 'data-clear-content.js', 'core-data.js', 'data-satori.js', 'data-ui-text.js', 'data-board-quiz.js', 'data-assets.js', 'runtime-settings.js', 'runtime-audio.js', 'runtime.js', 'speed-runtime.js', 'board-animation.js', 'board-ui.js', 'commands-board.js', 'commands-progression.js', 'quiz-ui.js', 'clear-flow.js', 'message-ui.js', 'progression-render.js', 'master-dialog.js', 'progression-ui.js', 'ui-render.js', 'app-events.js', 'namespace-api.js'];
...
const moduleLabels = ['盤面ドメイン', 'クリア後メッセージデータ', '基礎データ', '悟り出題データ', '多言語UIテキスト', '盤面クイズデータ', '固定挿絵・SVGデータ', '実行設定', 'サウンド', '実行状態', 'スピードランランタイム', '盤面アニメーション補助', '盤面UI', '盤面コマンド', '進行コマンド', 'クイズUI', 'クリアフロー', 'メッセージUI', '進行表示', '節目ダイアログ', '進行UI', '画面描画境界', 'イベントと起動', '公開API名前空間'];
